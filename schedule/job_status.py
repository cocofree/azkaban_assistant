...
class JobStatus(object):
    ...
    def get_attr_list():
        ...
        attr_list = ['id','job_name','execute_time','exec_id','create_time','start_time','update_time','status']
        return attr_list
    ...
    @staticmethod
    def get_jobstatus_json(job_name,execute_time):
        job = JobStatus.get_jobstatus(job_name,execute_time)
        if job == None:
            return '{}'
    ...
    def update_starttime(self):
        sql='''
        update kk_jobs_status
        set start_time=now()
        where job_name='%s' and execute_time=%s
        ''' % (self.job_name,self.execute_time,)
        db = get_db()
        db.execute(sql)
...
if __name__=='__main__':
    ...
    '''
    print JobStatus.get_jobstatus_json('udw_ml_tag_normalize',20161220)
    #jobs = []
    #for i in range(5):
    #    v = 2015121415-i
    #    print v
