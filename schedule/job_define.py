...
class Job(object):
    ...
    @staticmethod
    def get_projectid_byname(project_name):
        sql = '''
        select id
        from projects
        where name='%s'
        ...
        ret = db.fetch_one(sql)
        if ret==None:
            raise MyScheduleException('找不到项目[%s]' % project_name)
        else:
            return ret[0]
    ...
    def get_time_type(self):
        time_types = ['cur_hour','cur_date','last_hour','last_two_hour','last_date','last_two_date']
        for t in time_types:
            if self.server_script.find('&[%s]' % t)>=0:
                return t
        return 'cur_date'
    ...
    def has_job_permission(self):
        if self.updater == '':
            return False,'updater is null'

        projects = self.get_projects(self.updater)
        if self.project_name not in set(projects):
            return False,'[%s] Permission denied' % self.project_name

        try:
            ori_job = self.get_job_fromdb(self.name)
            if ori_job.project_name not in set(projects):
                return False,'[%s] source project [%s] Permission denied' % (self.name,ori_job.project_name)
        except:
            pass
        return True,''
    ...
    def unschedule_flow(self,session_id=None):
        try:
            sql = 'select trigger_id,data from triggers'
            db = get_db()
            rets = db.fetch_all(sql)
            for trigger_id,data in rets:
                #print trigger_id
                inbuffer = StringIO.StringIO(data)
                f = gzip.GzipFile(mode="rb", fileobj=inbuffer)
                rdata = f.read()
                f.close()
                m = json.loads(rdata)
                flow_name = m['actions'][0]['actionJson']['flowName']
                project_name = m['actions'][0]['actionJson']['projectName']
                #print flow_name,project_name
                #logging.info(flow_name,project_name)
                
                if self.name==flow_name:
                    if session_id==None:
                        session_id = get_session_id('admin','admin_pwd')
                    print trigger_id,flow_name,project_name
                    cmd = 'curl -k -i https://localhost:8443/schedule -d "action=removeSched&scheduleId=%s" -b azkaban.browser.session.id=%s' % (trigger_id,session_id)
                    print cmd
                    logging.info(cmd)
            ...
            print e
            logging.info(str(e))
        #cmd = 'curl -k https://localhost:8443/schedule -d "flowName=%s&action=removeSched&scheduleId=SCHEDULE_ID" -b azkaban.browser.session.id=%s' % (self.name,session_id)
        #logging.info(delete_sql)
        #status,result = commands.getstatusoutput(cmd)
